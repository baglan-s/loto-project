        ...
        <div class="row">
            <div class="col-md-12">
                <div class="overview-wrap m-b-40">
                    <h2 class="title-1">Изменение приза</h2>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <div class="card-body card-block">
                        <form action="{{ route('present.update', $present->id) }}" method="post" class="form-horizontal">
                            {{ csrf_field() }}
                                        ...
                                        <div class="row">
                                            <div class="col-md-6 col">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" id="hf-name" name="name" placeholder="Название приза" class="form-control" value="{{ $present->name }}" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col">
                                                <div class="form-group">
                                                    <label for="hf-amount" class=" form-control-label">Общее количество</label>
                                                    <input type="number" id="hf-amount" name="general_amount" class="form-control" value="{{ $present->amount }}" required>
                                                </div>
                                            </div>
                                            <div class="col-md-6 col">
                                                <div class="form-group">
                                                    ...
                                                    <select name="category" id="hf-category" class="form-control">
                                                        @if (!empty($present_categories))
                                                            @foreach($present_categories as $category)
                                                                <option value="{{ $category->id }}" @if($present->category->id == $category->id) selected @endif>{{ $category->name }}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                </div>
                                                ...
                                                <ul class="list-group list-group-flush">
                                                    @foreach($regions as $region)
                                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                                            ...
                                                            <input type="number" name="region_amount_{{ $region->id }}" class="number-input-visible p-1" style="border: 1px solid #ccc" value="{{ $region->presents()->find($present->id)->pivot->region_amount ?? 0 }}">
                                                        </li>
                                                    @endforeach
